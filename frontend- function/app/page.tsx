  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // 如果 API 失败或数据不完整，则直接从 public 读取并在前端映射
        if (!card || !card.title) {
          try {
        ...
        if (detailRes.ok) {
          ...
          if (card) card.risk_types = detail.risk_types || []
        } else {
          try {
            const raw = await fetch("/data/douyin/detail.json", { cache: "no-store" })
            const json = await raw.json()
            const riskSet = new Set<string>()
            ;(json.result?.timeline || []).forEach((t: any) => (t.risk_type || []).forEach((r: string) => riskSet.add(r)))
            if (card) card.risk_types = Array.from(riskSet)
          } catch {}
        }
        if (!card) return
        setVideos((prev) => {
