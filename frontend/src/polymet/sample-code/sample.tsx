...
interface HandlingSuggestionsData {
  overview: string;
...
interface HandlingSuggestionsProps {
...
export default function HandlingSuggestions({
  data,
  isOpen,
  onClose,
  className = "",
}: HandlingSuggestionsProps) {
  ...
  return (
    <div
      className={`w-full max-w-6xl mx-auto rounded-3xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl ${className}`}
    >
      {/* Header */}
      ...
      <div className="p-6">
        <div className="space-y-6">
          ...
          <Card className="bg-white/10 backdrop-blur-xl border-white/20">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <Badge
                    className={`px-3 py-1 text-xs font-medium border rounded-full ${getSeverityColor(
                      data.severity
                    )}`}
                  >
                    {data.severity === "high"
                  ...
                  <Badge
                    className={`px-3 py-1 text-xs font-medium border rounded-full ${getPriorityColor(
                      data.priority
                    )}`}
                  >
                    {data.priority}优先级
          ...
          <Card className="bg-white/10 backdrop-blur-xl border-white/20">
            ...
            <CardContent>
              <Tabs
                value={activeStrategy.toString()}
                onValueChange={(value) => setActiveStrategy(parseInt(value))}
              >
                <TabsList className="grid w-full grid-cols-2 bg-white/10 backdrop-blur-xl border border-white/20">
                  {data.strategies.map((strategy, index) => (
                    <TabsTrigger
                      ...
                      className="flex items-center space-x-2 data-[state=active]:bg-white/20"
                    >
                      {strategy.tone === "good" ? (
                      ...
                      <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => copyToClipboard(strategy.talk_track)}
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {data.outreach_channels.map((channel, index) => (
                  <div
                    key={index}
                    className="p-3 rounded-lg bg-white/5 border border-white/10 flex items-center space-x-2"
                  >
