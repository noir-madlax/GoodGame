...
export default function SeverityAnalysis({
  ...
  // 显示层文案映射（不改变原有分组与逻辑值）
  const displaySeverity = (s: string) => (s === "未标注" ? "未知优先级" : s);
                ...
                .map(([creator, count]) => (
                  <div key={creator} className="flex justify-between items-center">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-gray-600">{displayCreator(creator)}:</span>
                    </div>
          ...
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <XAxis dataKey="severity" stroke="#6b7280" fontSize={12} tick={{ fontSize: 12 }} tickFormatter={(v) => displaySeverity(String(v))} />
