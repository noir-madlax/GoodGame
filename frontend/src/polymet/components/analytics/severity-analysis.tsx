...
export default function SeverityAnalysis({
  ...
  const platformColors = { 抖音: "#ff6b6b", 小红书: "#ff8cc8", 其他: "#74c0fc" } as const;
      ...
      return (
        <div className="bg-white/95 backdrop-blur-xl border border-white/20 rounded-xl p-4 shadow-2xl max-w-xs">
          ...
          <div className="mt-3 pt-3 border-t border-gray-200">
            ...
            <div className="space-y-1">
              {[{ name: "抖音", count: Math.floor(severityData.total * 0.6) }, { name: "小红书", count: Math.floor(severityData.total * 0.4) }].map(({ name, count }) => (
                <div key={name} className="flex justify-between items-center">
                  <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: platformColors[name as keyof typeof platformColors] }} /><span className="text-gray-600">{name}:</span></div>
    ...
    <div className={`space-y-6 ${className}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">"{relevanceType}" 严重程度分析</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">点击柱状图查看对应视频列表</p>
          </div>
        </div>
        <div className="text-right"><div className="text-2xl font-bold text-gray-900 dark:text-white">{totalCount}</div><div className="text-sm text-gray-600 dark:text-gray-400">{relevanceType}视频数</div></div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <Bar dataKey="total" radius={[4, 4, 0, 0]} onClick={handleBarClick} className="cursor-pointer" label={{ position: "inside", fill: "#ffffff", fontSize: 12, fontWeight: "bold", formatter: (value: number, entry: any) => (entry && entry.percentage !== undefined ? `${entry.percentage}%` : `${value}`) }}>
                  {chartData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={severityColors[entry.severity as keyof typeof severityColors]} stroke={selectedSeverity === entry.severity ? "#ffffff" : "transparent"} strokeWidth={selectedSeverity === entry.severity ? 2 : 0} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between"><h4 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2"><Filter className="w-4 h-4" />严重程度统计</h4>{data.length > 2 && (<Button variant="ghost" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">{isExpanded ? "收起" : `展开全部 (${data.length})`}</Button>)}</div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {Object.entries(item.creators).map(([creator, count]) => (
                      <div key={creator} className="text-center p-2 rounded-lg bg-white/5 border border-white/10 cursor-pointer hover:bg-white/10 transition-colors" onClick={(e) => { e.stopPropagation(); onSeverityClick?.(item.severity, creator); }}>
                        <div className="w-2 h-2 rounded-full mx-auto mb-1" style={{ backgroundColor: creatorColors[creator as keyof typeof creatorColors] }} />
