...
5. **完整保留原始分析数据的所有细节和颗粒度**

报告结构要求：
- 报告标题和概述
- 视频文件基本信息
- 总体相似度评估
- 各维度详细分析（旋律、节奏、音色、人声、结构）
- 差异分析
- 音频特征对比
- 内容重叠分析
- 综合结论和专业意见
...
音频1来源：
- 文件名：{video1_info.get('filename', 'N/A')}
- 文件大小：{video1_info.get('size_mb', 0)} MB
- 时长：{video1_info.get('duration', 'N/A')}

音频2来源：
- 文件名：{video2_info.get('filename', 'N/A')}
...
def create_pdf_report(
    ...
    # === 优先展示视频信息和截图 ===
    story.append(Paragraph("视频文件信息", heading_style))
    story.append(Spacer(1, 0.5*cm))
    
    # 视频信息表格
    video_data = [
        ['', '音频来源1', '音频来源2'],
        ['文件名', video1_info.get('filename', 'N/A'), video2_info.get('filename', 'N/A')],
    ...
    video_table = Table(video_data, colWidths=[4*cm, 6*cm, 6*cm])
...
def main():
    ...
    # 输入文件
    analysis_json = output_dir / "audio_comparison_analysis.json"
    ...
    # 输出文件
    report_text_file = output_dir / "audio_analysis_report.txt"
    report_pdf_file = output_dir / "audio_analysis_report.pdf"
    video1_thumbnail = output_dir / "video1_thumbnail.jpg"
    video2_thumbnail = output_dir / "video2_thumbnail.jpg"
    ...
    print(f"✓ 视频文件1: {video1_filename}")
    print(f"✓ 视频文件2: {video2_filename}\n")
    ...
    # 步骤 0: 提取视频信息和截图
    print("步骤 0: 提取视频信息和截图")
    print("-" * 80)
    
    video1_info = get_video_info(video1_path)
    video2_info = get_video_info(video2_path)
    
    log.info(f"视频1: {video1_info['filename']}, {video1_info['size_mb']} MB, {video1_info['duration']}")
    log.info(f"视频2: {video2_info['filename']}, {video2_info['size_mb']} MB, {video2_info['duration']}")
    
    # 提取视频截图
    extract_video_thumbnail(video1_path, video1_thumbnail)
    extract_video_thumbnail(video2_path, video2_thumbnail)
    print()
    ...
    try:
        ...
        report_text = generator.generate_report_text(
            ...
            video1_info=video1_info,
            video2_info=video2_info
        )
        print()
        ...
        print(f"🖼️  视频截图1: {video1_thumbnail}")
        print(f"🖼️  视频截图2: {video2_thumbnail}\n")
