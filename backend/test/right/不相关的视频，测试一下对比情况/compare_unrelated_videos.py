...
# Gemini 配置
GEMINI_MODEL_FLASH = "gemini-2.5-flash"  # 用于视频分析
...
请保持客观、专业、详细，基于视频内容进行分析。"""
    ...
    def _build_user_prompt(self, video1_name: str, video2_name: str) -> str:
        ...
        Returns:
            用户提示词
        """
        return f"""请仔细对比分析以下两个视频，特别注意这两个视频是否属于完全不同的内容类型。判断第二个视频是否涉嫌抄袭或搬运第一个视频的内容。
...
**重要提醒：**
- 这两个视频可能是完全不同类型的内容（如脱口秀 vs 真人秀节目）
- 请仔细区分每个视频的实际内容，不要混淆
- 如果两个视频属于不同类型的内容，相似度应该很低
...
{{
  ...
  "content_similarity": {{
    "score": 0-100,
    "description": "内容相似度的详细描述，包括视频类型和主题的比较",
    "evidence": [
      "证据1：具体的相似场景、情节或对话",
      "证据2：...",
      "..."
...
**重要提示：**
1. 请逐帧、逐段分析视频内容，仔细识别每个视频的实际内容
2. 特别关注视频类型：是否为脱口秀、真人秀、纪录片、电影等不同类型
3. 关注细节：场景、人物、对话、音乐、特效等
4. 识别任何修改手法：裁剪、镜像、变速、调色、加字幕、加水印等
5. 评估是否为简单搬运，还是有实质性的二次创作
6. 如果两个视频属于完全不同类型的内容，相似度应该接近0
7. 提供充分的证据和推理过程
8. 保持客观、专业、详细
...
def main():
    """主函数"""
    # 视频文件路径
    video1_path = Path("/Users/rigel/project/hdl-tikhub-goodgame/backend/test/right/music/output/bilibili_music_video.mp4")
    video2_path = Path("/Users/rigel/project/hdl-tikhub-goodgame/backend/test/right/video-compare-张雨绮/video1_original.mp4")

    # 输出目录
    output_dir = Path("/Users/rigel/project/hdl-tikhub-goodgame/backend/test/right/不相关的视频，测试一下对比情况/output/test3-不同类型视频对比测试")
    output_dir.mkdir(parents=True, exist_ok=True)
