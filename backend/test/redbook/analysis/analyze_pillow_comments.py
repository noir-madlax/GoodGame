...
从整体视角提取消费者洞察，帮助商家优化产品和营销策略。

数据规模：44 个帖子，7149 条评论
预估 Token：约 35 万（远低于 Gemini 2.5 Flash 的 100 万上限）

使用方法：
    python analyze_pillow_comments.py
"""
...
def load_prompt() -> str:
    """加载分析 prompt"""
    prompt_path = Path(__file__).parent / 'pillow_comment_analysis_prompt.txt'
...
def main():
    ...
    try:
        ...
        output_file = output_dir / f"pillow_analysis_{timestamp}.json"
        ...
        # 9. 打印摘要
        print(f"\n{'='*60}", flush=True)
        print("📊 分析摘要", flush=True)
        print(f"{'='*60}", flush=True)
        
        if 'key_takeaways' in result:
            print("\n🔑 核心发现:", flush=True)
            for i, takeaway in enumerate(result['key_takeaways'][:5], 1):
                print(f"   {i}. {takeaway}", flush=True)
        
        if 'user_concerns_ranking' in result:
            print("\n📌 用户关注点 TOP 5:", flush=True)
            for item in result['user_concerns_ranking'][:5]:
                print(f"   {item.get('rank', '-')}. {item.get('concern', 'N/A')} ({item.get('percentage', 'N/A')})", flush=True)
        
        if 'hot_questions_faq' in result:
            print("\n❓ 热门问题 TOP 3:", flush=True)
            for item in result['hot_questions_faq'][:3]:
                print(f"   - {item.get('question_category', 'N/A')}", flush=True)
                if 'typical_questions' in item:
                    for q in item['typical_questions'][:2]:
                        print(f"     • {q}", flush=True)
        
        if 'market_opportunities' in result:
            print("\n💡 市场机会:", flush=True)
            for item in result['market_opportunities'][:3]:
                print(f"   - {item.get('opportunity', 'N/A')}", flush=True)
        
        print(f"\n{'='*60}", flush=True)
        print("✅ 分析完成！", flush=True)
        print(f"   完整结果请查看: {output_file}", flush=True)
        print(f"{'='*60}", flush=True)
