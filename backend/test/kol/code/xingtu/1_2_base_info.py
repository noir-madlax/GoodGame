...
功能: 获取KOL的基础信息数据，包括账号状态、粉丝数、认证信息、擅长领域等
...
def get_kol_base_info(api_key: str, kol_id: str, cookie: str):
    """
    调用基础信息接口
    ...
    # 正确参数: kolId（驼峰） + platformChannel=_10（直播）
    params = {
        'kolId': kol_id,
        'platformChannel': '_10'  # _10=直播, _1=短视频（但_1会返回400）
    }
...
def main():
    """主函数"""
    interface_name = 'base_info'
    
    print('='*70)
    print('接口 1.2: 获取KOL基础信息')
    print('='*70)
    ...
    # 5. 逐个处理KOL
    print(f'\n4️⃣ 开始获取基础信息数据...')
    print(f'  参数: kolId + platformChannel=_10 (直播)')
    print('='*70)
    ...
    for idx, kol in enumerate(pending_kols, 1):
        ...
        # 调用接口
        result = get_kol_base_info(api_key, kol['xingtu_kol_id'], cookie)
