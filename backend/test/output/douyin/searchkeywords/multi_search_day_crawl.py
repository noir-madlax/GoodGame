...
MAX_PAGES = 200            # 安全上限，避免无限循环，可按需调整
# ==================================================
...
def parse_next_page_info(page_payload: Dict[str, Any]) -> Tuple[int, Optional[int], Optional[str]]:
	"""解析分页信息，返回 (has_more, cursor, search_id)。

	遍历 data.data 数组，读取 business_config.next_page 与 has_more。
	"""
	has_more = 0
...
def build_first_payload(keyword: str) -> Dict[str, Any]:
	"""构建第一页请求体（cursor=0, search_id=''，并设置 publish_time=1 等筛选）。"""
	return {
		"keyword": keyword,
		"cursor": 0,
		"sort_type": SORT_TYPE,
...
def crawl_all_pages(keyword: str) -> Dict[str, Any]:
	"""从第一页开始抓取，按 has_more 翻页直至结束或达到 MAX_PAGES。"""
	...
	requests_count += 1
	page_count += 1
	path = save_page(output_dir, "douyin_multi_search_day", page_count, page)
...
def main() -> None:
	"""执行按天筛选的循环抓取，并打印汇总结果（页数、请求次数、文件列表）。"""
