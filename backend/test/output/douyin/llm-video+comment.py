...
def analyze_with_local_file(client: genai.Client, local_path: Path, model: str) -> types.GenerateContentResponse:
	...
	prompt_path = Path(__file__).resolve().parent / "video+comment-prompt.txt"
	...
	return client.models.generate_content(
		...
		contents=[
			video_part,
			"请基于系统提示对视频和评论进行舆情与合规风险分析，并严格输出 JSON。",
		],
...
def analyze_with_inputs(
	...
	contents.append("请基于系统提示对视频与评论进行舆情与合规风险分析，并严格输出 JSON。")
