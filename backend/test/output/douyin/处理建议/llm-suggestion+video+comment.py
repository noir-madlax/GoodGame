...
严格按照同目录下的 system prompt（suggestion+video+comment-prompt.txt）执行，不在代码中追加任何分析性指令。"""
#!/usr/bin/env python3
import os
import json
import time
...
class StrategyItem(BaseModel):
	tone: str  # "good" | "strict"
	target: str | None = None  # "发布者" | "评论者"
	talk_track: str
	incentives: list[str] | None = None
	legal_basis: list[LegalBasisItem] | None = None
	platform_policy_basis: list[PolicyBasisItem] | None = None
	required_actions: list[str] | None = None
	escalation_path: str | None = None


class HandlingSuggestions(BaseModel):
	overview: str
	severity: str  # low | medium | high
	priority: str  # P3 | P2 | P1
	legal_risk_summary: str | None = None
	strategies: list[StrategyItem]
	outreach_channels: list[str] | None = None
	follow_up_checklist: list[str] | None = None
	redact_suggestions: list[str] | None = None


class VideoAnalysisWithSuggestions(VideoAnalysisV3):
	handling_suggestions: HandlingSuggestions | None = None
...
def analyze_with_local_file(client: genai.Client, local_path: Path, model: str) -> types.GenerateContentResponse:
	...
	# Load system prompt only from txt next to this script
	prompt_path = Path(__file__).resolve().parent / "suggestion+video+comment-prompt.txt"
	...
	return client.models.generate_content(
		...
		config=types.GenerateContentConfig(
			...
			response_schema=VideoAnalysisWithSuggestions,
...
def analyze_with_inputs(
	...
	contents: list[types.Part | str] = [video_part]
